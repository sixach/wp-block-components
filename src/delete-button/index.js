 ...
 * to allow deleteing an item from the existing list of items.
 *
 * @function
 * @since	   1.5.0
...
function DeleteButton( { className, children, onClick } ) {
	return (
		<Button className={ className } isDestructive icon={ trash } isLink label={ __( 'Delete', 'sixa' ) } showTooltip onClick={ onClick }>
			{ children }
		</Button>
	);
}
